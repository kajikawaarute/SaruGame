...
void Skeleton::InitBoneMatrixArrayStructuredBuffer()
{
	...
	//StructuredBufferを作成。
	GetD3DDeviceGraphicsEngine()->CreateBuffer(&desc, NULL, &m_boneMatrixSB);
}
void Skeleton::InitBoneMatrixArrayShaderResourceView()
{
	...
	GetD3DDeviceGraphicsEngine()->CreateShaderResourceView(m_boneMatrixSB, &desc, &m_boneMatrixSRV);
}
...
void Skeleton::SendBoneMatrixArrayToGPU()
{
	...
	//StructuredBufferを更新。
	GetD3DDeviceContextGraphicsEngine()->UpdateSubresource(
		m_boneMatrixSB, 0, NULL, &m_boneMatrixs.front(), 0, 0
	);
	//ボーン行列を頂点シェーダーステージに設定。
	GetD3DDeviceContextGraphicsEngine()->VSSetShaderResources(enSkinModelSRVReg_BoneMatrixArray, 1, &m_boneMatrixSRV);
